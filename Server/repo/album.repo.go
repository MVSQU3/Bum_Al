...
func (r *AlbumRepository) GetById(id int) (*models.Album, error) {
	query := "SELECT id, title, artist, year, cover_url FROM albums WHERE id = $1"
	var album models.Album

	err := r.db.QueryRow(query, id).Scan(&album.ID, &album.Title, &album.Artist, &album.Year, &album.Cover_url)
...
func (r *AlbumRepository) Add(album *models.Album) (*models.Album, error) {
	query := "INSERT INTO albums (title, artist, year, cover_url) VALUES ($1, $2, $3, $4) RETURNING id"
